...
if(!class_exists('WP_Adress_Schema_Display')) {
	...
	class WP_Address_Schema_Display extends Wp_Address_Schema
	{
		...
		public function parseAddress($id) {
			...
			$schema_classification = "company-address";
			$display_closed = @get_post_meta($id, 'display_closed', true);
			
			$string = '<'.(($seperate=='<li>')?'ul':'div').' itemscope class="'.$schema_classification.'">';
			...
			if((isset($add1) && $add1 != '') || (isset($add2) && $add2 != '') 
				...
				|| (isset($pcode) && $pcode != '')) 
			{
				$string .= '<'.(($seperate=='<li>')?'li><ul':'div').' itemprop="address" itemscope class="PostalAddress">';
				$street_addr = array();
				...
				if(!empty($street_addr)) $string .= $this->parseLine('<span class="streetAddress">'.$street_address.'</span>', $seperate, 'street_address');
				if(isset($city) && $city != '') $string .= $this->parseLine('<span class="addressLocality">'.$city.'</span>', $seperate, 'address_locality');
				if(isset($county) && $county != '') $string .= $this->parseLine('<span class="addressRegion">'.$county.'</span>', $seperate, 'address_region');
				if(isset($pcode) && $pcode != '') $string .= $this->parseLine('<span class="postalCode">'.$pcode.'</span>', $seperate, 'postal_code');
